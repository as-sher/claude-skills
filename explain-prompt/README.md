...
## Case study: financial AI assistant — educational system prompt

A multi-agent fintech platform runs separate specialist agents for different intent categories. One agent handles educational financial queries (concept explanations, comparisons, regulatory context). Its system prompt had accumulated over several iterations of product requirements.

**Prompt inventory (via `/explain-prompt`)**
...
─────────────────────────────────────────────────────────
 ...
 ─────────────────────────────────────────────────────────
 Persona / Introduction           68     6.1%
 Knowledge Blueprint             163    14.7%
   └── Structure definition       70
   └── Embedded format example    93    ← partial few-shot
 Answering Philosophy            135    12.2%
 Content Guidelines              135    12.2%   ⚠ DUPLICATE
 Answer Structure                 90     8.1%
 Data Integrity & Safety          90     8.1%
 Language instruction             20     1.8%
 Engagement & Interaction         50     4.5%
 Closing summary sentence         33     3.0%   ← zero signal
 Markdown headers / whitespace    80     7.2%
 ─────────────────────────────────────────────────────────
 TOTAL                         1,113   100.0%
 Cache status: static — already cached correctly
```

**Issues found**

- `Answering Philosophy` and `Content Guidelines` were near-identical in intent — both instructed the model to explain concepts with examples in Indian market context. 150 tokens of redundancy, 13% of the prompt.
- An embedded format example (~93 tokens) used bracket placeholders instead of real content, demonstrating structure the preceding numbered list already defined. It added tokens without adding signal.
- A closing sentence ("The assistant ensures each answer is comprehensive...") was self-description rather than instruction — 33 tokens with zero behavioral value.
- `"Cover all important aspects without artificial length constraints"` was an active anti-bound instruction. At p75, educational responses ran 1,500+ tokens. The downstream formatter agent already defined target word counts by query type; that guidance was not reflected upstream.
...
```
 ...
 Output bounded by query type:
   Lookup query     →  200–350 words
   Comparison       →  350–500 words
   Deep concept     →  400–600 words
```

**Operational impact at 10,000 calls/day on Sonnet 4.6**

| | Daily cost |
|---|---|
| Before (cached, unbounded output) | $139.80 |
| After (cached, bounded output) | $69.90 |
| Annual delta | ~$25,500 |

The output savings came entirely from adding 20 tokens of word-count guidance — not from compressing the prompt itself. The most expensive line was the instruction telling the model not to bound its output.
